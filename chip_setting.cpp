...
chip_setting::chip_setting(QWidget *parent, double cl, double cw, double cb,
                           double cpl, double cpw,
                           double de1l, double de1w, double de2l, double de2w,
                           double des, double cps, double lw) : QDialog(parent), ui(new Ui::chip_setting)
{
    ui->setupUi(this);
    chip_length_mm = cl;
    chip_width_mm = cw;
...
{
    if(!ui->chip_length->text().isEmpty()) chip_length_mm = ui->chip_length->text().toDouble();
    if(!ui->chip_width->text().isEmpty()) chip_width_mm = ui->chip_width->text().toDouble();
    if(!ui->chip_border->text().isEmpty()) chip_border_mm = ui->chip_border->text().toDouble();

    if(!ui->cp_length->text().isEmpty()) cp_length_mm = ui->cp_length->text().toDouble();
    if(!ui->cp_width->text().isEmpty()) cp_width_mm = ui->cp_width->text().toDouble();

    if(!ui->de1_length->text().isEmpty()) de1_length_mm = ui->de1_length->text().toDouble();
    if(!ui->de1_width->text().isEmpty()) de1_width_mm = ui->de1_width->text().toDouble();
    if(!ui->de2_length->text().isEmpty()) de2_length_mm = ui->de2_length->text().toDouble();
    if(!ui->de2_width->text().isEmpty()) de2_width_mm = ui->de2_width->text().toDouble();

    if(!ui->de_spacing->text().isEmpty()) de_spacing_mm = ui->de_spacing->text().toDouble();
    if(!ui->cp_spacing->text().isEmpty()) cp_spacing_mm = ui->cp_spacing->text().toDouble();
    if(!ui->line_width->text().isEmpty()) line_width_mm = ui->line_width->text().toDouble();
    emit reset(this);
    this->close();
}
